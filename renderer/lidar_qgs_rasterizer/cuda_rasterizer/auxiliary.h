...
// Channel-layout constants moved to channel_layout.h (host-safe). Anything
// that was built on top of NORMAL_OFFSET / DEPTH_OFFSET / ... still resolves
// because that header is unconditionally included above.
...
{
	rect_min = {p.x - max_radius.x, p.y - max_radius.y};
	rect_max = {
		p.x + max_radius.x,
		p.y + max_radius.y
	};
...
__forceinline__ __device__ double3 transformPoint4x3_without_t_double(const double3& p, const float* matrix)
{
	double3 transformed = {
		matrix[0] * p.x + matrix[4] * p.y + matrix[8] * p.z,
		matrix[1] * p.x + matrix[5] * p.y + matrix[9] * p.z,
...
__forceinline__ __device__ float GetParabolaA(const float2 cos2_sin2, const float3 rs_2_sign, const float3 scale){

	float first = cos2_sin2.x * rs_2_sign.x;
