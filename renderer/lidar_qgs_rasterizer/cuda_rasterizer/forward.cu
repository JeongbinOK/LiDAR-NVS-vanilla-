...
// ---------------------------------------------------------------------------
// preprocessLidarCUDA (A3.2.c + A3.2.d)
//
// Panoramic LiDAR preprocess. Mirrors preprocessCUDA but replaces the
// perspective (cam) culling + projection path with spherical helpers from
...
// This is conservative (over-emits a few tiles in the far hemisphere) but
// correctness-preserving: per-tile ray-paraboloid tests reject non-
// contributing tiles during the render pass. Both preprocess and
// duplicateWithKeys read the same rects[]/points_xy[] buffers, so the
// tile-count accounting stays consistent without touching the duplicate
// kernel.
// ---------------------------------------------------------------------------
template<int C>
__global__ void preprocessLidarCUDA(int P, int D, int M,
	float* aabb,
	const float* orig_points,
	const glm::vec3* scales,
	const float scale_modifier,
	...
	bool prefiltered)
{
	...
	// --- View-space position (sensor frame) ---
	float3 p_orig = { orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2] };
	...
	// --- Per-primitive rscale + scales_sign (side effects the render kernel needs).
	//     This mirrors the first lines of computeAABB without doing pixel projection.
	{
		const glm::vec3 s = scales[idx];
		float4 ro;
		ro.x = __frcp_rn(s.x * s.x);
		ro.y = __frcp_rn(s.y * s.y);
		ro.z = __frcp_rn(s.z);
		ro.w = opacities[idx];
		rscales_opacity[idx] = ro;

		int3 ss;
		ss.x = (int)copysignf(1.0f, s.x);
		ss.y = (int)copysignf(1.0f, s.y);
		ss.z = (int)copysignf(1.0f, s.z);
		scales_sign[idx] = ss;
	}

	// --- Spherical AABB ---
	// Effective radius: sigma * max(|sx|, |sy|, |sz|) * scale_modifier.
	const glm::vec3 s = scales[idx];
	...
	bool wrapped;
	qgs_lidar::aabb_spherical(p_view.x, p_view.y, p_view.z, R_eff,
		az_min, az_max, el_lo, el_hi, wrapped);

	// Clamp elevation extent to sensor FOV.
	el_lo = fmaxf(el_lo, el_min);
	el_hi = fminf(el_hi, el_max);
	...
	// same (center, extent) pair, so tile counts agree automatically.
	float u_c, u_extent;
	if (wrapped)
	{
		u_c      = 0.5f * (float)W;
		u_extent = 0.5f * (float)W;
	}
	else
	{
		u_c = (az_c + qgs_lidar::PI_F) * w_per_rad_az;
		const float u_lo = (az_min + qgs_lidar::PI_F) * w_per_rad_az;
		const float u_hi = (az_max + qgs_lidar::PI_F) * w_per_rad_az;
		u_extent = fmaxf(fabsf(u_hi - u_c), fabsf(u_c - u_lo));
		// Reject if bbox lies entirely outside the image horizontally.
		if (u_hi < 0.0f) return;
		if (u_lo > (float)W) return;
	}

	float2 center = { u_c, v_c };
	float2 extent = { u_extent, fmaxf(fabsf(v_hi - v_c), fabsf(v_c - v_lo)) };

	// Reject vertically-OOB boxes (u already handled above).
	if (v_hi < 0.0f) return;
	if (v_lo > (float)H) return;
	...
	// Colors: LiDAR mode always uses colors_precomp — SH path is camera-only
	// (direction-dependent colour is not meaningful for active sensing).
	if (colors_precomp == nullptr)
	{
		// Clamp to zero (signal it was unprojected).
		#pragma unroll
		for (int c = 0; c < C; ++c) rgb[idx * C + c] = 0.0f;
	}

	depths[idx]         = r_sph;          // range, not z — downstream is a scalar anyway
	rects[idx]          = extent;
	points_xy_image[idx] = center;
	radii[idx]          = (int)fmaxf(ceilf(extent.x), ceilf(extent.y));
	tiles_touched[idx]  = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x);

	aabb[4 * idx + 0] = (float)rect_min.x;
	aabb[4 * idx + 1] = (float)rect_min.y;
	aabb[4 * idx + 2] = (float)rect_max.x;
	aabb[4 * idx + 3] = (float)rect_max.y;
}
...
renderCUDA(
	...
	// A3.2.d — LiDAR mode (see resorted_render.cuh for details).
	...
	const float h_per_rad_el)
{
	...
	// create the direction (camera vs LiDAR; see resorted_render.cuh).
	float3 ray_point;
	if (lidar_mode) {
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (return_normal){ // and curvature
				...
				// normal — NORMAL_DIM=3 (xyz), independent of CHANNELS=NUM_CHANNELS.
				for (int ch = 0; ch < NORMAL_DIM; ch++)
...
void FORWARD::preprocessLidar(int P, int D, int M,
	float* aabb,
	const float* means3D,
	const glm::vec3* scales,
	const float scale_modifier,
	...
	bool prefiltered)
{
	preprocessLidarCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		aabb,
		means3D,
		scales,
