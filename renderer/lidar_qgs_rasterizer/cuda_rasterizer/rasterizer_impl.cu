	...
	float r_far)
{
	...
	// In LiDAR mode cam_intr is repurposed as the spherical intrinsics —
	// pinhole focal/principal slots become garbage. Keep them out of the
	// backward render kernel by zeroing them; the kernel branches on
	// lidar_mode and ignores them entirely on that path.
	const float focal_x      = lidar_mode ? 0.0f : cam_intr[0];
	const float focal_y      = lidar_mode ? 0.0f : cam_intr[1];
	const float principal_x  = lidar_mode ? 0.0f : cam_intr[2];
