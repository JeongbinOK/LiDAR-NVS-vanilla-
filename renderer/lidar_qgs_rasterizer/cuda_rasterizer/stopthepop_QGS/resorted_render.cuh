...
__device__ __inline__ void updateMap(
	...
	float w = alpha * T;
    // normal — 3 components, not CHANNELS.
	...
	const float r_far)
{
	...
	// create the ray
	//  - Camera mode (lidar_mode=false): pinhole projection, ray_point = (x/f, y/f, 1).
	//                                    Intersection root t equals z-depth.
	//  - LiDAR mode  (lidar_mode=true):  spherical projection, ray_point is a unit
	//                                    direction d = (sin(az)cos(el), cos(az)cos(el), sin(el)).
	//                                    Intersection root t equals range r.
	float3 ray_point;
	if (lidar_mode) {
    ...
    float sort_normals[BUFFER_LENGTH * NORMAL_DIM];
    ...
    for (int i = 0; i < BUFFER_LENGTH; ++i)
	{
		...
		sort_alphas[i] = 0;
        sort_curvs[i] = 0;
        // sort_normals stride is NORMAL_DIM (=3), not CHANNELS.
        for (int ch = 0; ch < NORMAL_DIM; ch++)
            sort_normals[i * NORMAL_DIM + ch] = 0;
		sort_ids[i] = -1;
	}
    ...
    auto blend_one = [&](){
        ...
        for (int i = 1; i < BUFFER_LENGTH; ++i)
		{
			...
			if (return_normal){
				for (int ch = 0; ch < NORMAL_DIM; ch++)
                	sort_normals[(i - 1) * NORMAL_DIM + ch] = sort_normals[i * NORMAL_DIM + ch];
...
#pragma unroll
            for (int s = 0; s < BUFFER_LENGTH; s++){
                ...
                if (depth < sort_depths[s]){
					...
					if (return_normal){
						for (int ch = 0; ch < NORMAL_DIM; ch++)
							swap_T(normal[ch], sort_normals[s*NORMAL_DIM + ch]);
	...
	const float r_far)
{
	...
	// Mirror forward kernel ray construction: spherical in LiDAR mode, pinhole
	// otherwise. The downstream ray-paraboloid math treats `ray_point` as a
	// constant per-pixel direction (no gradient flows back into pixel coords),
	// so the only branch needed in backward is this one.
	float3 ray_point;
	if (lidar_mode) {
		const float az = pixf.x / w_per_rad_az - qgs_lidar::PI_F;
		const float el = pixf.y / h_per_rad_el + el_min_rad;
