...
def voxel_seeding(xyz: torch.Tensor, voxel_size: float) -> torch.Tensor:
    ...
    Selects one seed per occupied voxel (closest to voxel centroid).
...
def farthest_point_sampling(xyz: torch.Tensor, n_samples: int) -> torch.Tensor:
    """Farthest point sampling with torch_cluster acceleration.
    ...
    try:
        ...
        batch = torch.zeros(M, dtype=torch.long, device=xyz.device)
        idx = fps(xyz, batch, ratio=ratio, random_start=True)
        # fps may return slightly more or fewer points; truncate/pad
        if idx.shape[0] >= n_samples:
            return idx[:n_samples]
        # Rare: pad with remaining points
        remaining = torch.tensor(
            [i for i in range(M) if i not in idx.tolist()],
            device=xyz.device,
        )
        return torch.cat([idx, remaining[: n_samples - idx.shape[0]]])
    except ImportError:
        pass

    # Fallback: Python loop FPS
    ...
    for i in range(1, n_samples):
        last = xyz[indices[i - 1]].unsqueeze(0)
        d = (xyz - last).pow(2).sum(dim=1)
...
def compute_seeds(
    ...
    voxel_seeds = voxel_seeding(xyz, voxel_size)
